  ...
  return (
    <div className="relative mb-7">
      ...
      <div className="flex items-center gap-2 mb-1.5">
        <span aria-hidden className="font-mono text-[9px] text-mw-steel/70">
          {">>"}
        </span>
        <span className="font-mono text-[9px] uppercase tracking-[0.3em] text-mw-hazard">
          {label}
        </span>
        <span aria-hidden className="flex-1 h-px bg-gradient-to-r from-mw-steel/40 to-transparent" />
      </div>

      <div className="hud-clip-sm relative bg-mw-field/25 border-l-2 border-mw-hazard/50 pl-4 pr-4 py-3.5">
        ...
        <span aria-hidden className="absolute left-0 top-0 bottom-0 w-[2px] flex flex-col justify-around">
          {[0, 1, 2, 3].map((i) => (
            <span key={i} className="block w-[6px] h-px bg-mw-hazard/60" />
          ))}
        </span>
