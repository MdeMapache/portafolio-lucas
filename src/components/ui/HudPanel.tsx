...
export default function HudPanel({
  ...
  return (
    <div className="relative">
      {/* Capa de borde. */}
      <div className="hud-clip absolute inset-0 bg-mw-phosphor/45" />
      ...
      {[
        ...
        <span
          key={pos}
          aria-hidden
          className={`hud-rivet absolute ${pos} w-1.5 h-1.5 bg-mw-phosphor/70 text-mw-phosphor/40 z-10`}
        />
      ))}
      ...
      <div className="relative">
        ...
        <header className="flex items-stretch">
          {/* Placa del número. */}
          <span className="hud-tab flex items-center bg-mw-phosphor/85 pl-5 pr-6 py-1.5">
            <span className="font-mono text-[11px] font-bold text-mw-void tracking-widest">
              {code}
            </span>
          </span>

          <span className="flex items-center gap-3 flex-1 min-w-0 pl-3 pr-5 py-1.5 border-b border-mw-phosphor/25">
            <h2
              data-text={title}
        ...
        {/* Tira de estado --------------------------------------------------- */}
        <footer className="flex items-center gap-3 px-4 sm:px-7 py-1.5 border-t border-mw-phosphor/20">
          <span aria-hidden className="hazard-stripe-dim h-[7px] flex-1 opacity-60" />
          <span className="font-mono text-[8.5px] uppercase tracking-[0.2em] text-mw-phosphor/50 shrink-0">
